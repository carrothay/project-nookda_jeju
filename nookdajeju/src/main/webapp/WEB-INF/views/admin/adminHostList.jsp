...
<h2 class="title">호스트 목록</h2>
	<a href="adminRequestList.do" class="adbtn">신청 목록</a>
	<table class="adtable">
		<tr>
			...
			<th class="col-md-2 text-center">호스트 삭제</th>		
		</tr>
		<c:if test="${empty hostList }">
			<tr class="nonelist">
				<th colspan="6" class="text-center">호스트가 없습니다</th>
			</tr>
		</c:if>
		<c:if test="${not empty hostList }">
			<c:forEach var="member" items="${hostList }">
			<c:if test="${member.mhost == 'y' }">
				<tr>
					...
					<!-- 게시글 번호 순서 정렬 -->
						...
						<c:if test="${member.mhost == 'y' }">
								<td class="text-center">
									<a href="adminHostCancle.do?memail=${member.memail }&pageNum=${pageNum }">
										<img alt="" src="${path }/resources/img/hostdlt.svg" width="25px" height="25px">
									</a>
								</td>
							</c:if>
					</tr>
	...
	<div align="center">
		<ul class="pagination_bottom">
			...
			<c:if test="${pb.startPage > pb.pagePerBlock }">
				<li>
					<a href="adminHostList.do?pageNum=1">
						<img alt="" src="${path }/resources/img/left2.png">
					</a>
				</li>
				<li>
					<a href="adminHostList.do?pageNum=${pb.startPage - 1 }">
			...
			<!-- 현재 머물고 있는 페이지가 몇 페이지인지 구별할 때 -->
				<c:if test="${pb.currentPage == i }">
					<li class="currentPage">
						<a href="adminHostList.do?pageNum=${i }">${i }</a>
					</li>
				</c:if>
				<c:if test="${pb.currentPage != i }">
					<li>
			...
			<c:if test="${pb.endPage < pb.totalPage }">
				<li>
					<a href="adminHostList.do?pageNum=${pb.endPage + 1 }">
						<img alt="" src="${path }/resources/img/right.png">
					</a>
				</li>
				<li>
					<a href="adminHostList.do?pageNum=${pb.totalPage}">
