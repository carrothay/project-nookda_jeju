...
;(function($){
	...
	$.fn.tDatePicker=function(pr_el,options){
		...
		function AppendDaysInMonth(pr_num){
			...
			var setTr='';
			while(i<pr_num){
				setTr=setTr+'<tr>'+'<td class="t-day">1</td>'+'<td class="t-day">2</td>'+'<td class="t-day">3</td>'+'<td class="t-day">4</td>'+'<td class="t-day">5</td>'+'<td class="t-day">6</td>'+'<td class="t-day">0</td>'+'</tr>';i++;
			}
			return setTr;
		}
		
		function setThemeCheckDate(pr_title,pr_class,pr_data_utc,pr_input,pr_fm_input){
			return'<div class="t-dates t-date-'+pr_class+'">'+settings.iconDate+'<label class="t-date-info-title">'+pr_title+'</label>'+showThemeDate(pr_class,pr_data_utc)+'</div>'+'<input type="hidden" class="t-input-'+pr_class+'"'+' value="'+pr_fm_input+'" name="'+pr_input+'">'
		}
		...
		function showThemeDate(pr_class,pr_data_utc){
			if(pr_data_utc!==0&&pr_data_utc!==null){
				...
				if(settings.showDateTheme==='dd'){
					...
					return'<span class="t-year-'+pr_class+'"> '+check_num_10(d.getFullYear())+"년"+'</span>'+'<span class="t-month-'+pr_class+'"> '+showMonths+' </span>'+'<span class="t-day-'+pr_class+'"> '+check_num_10(d.getDate())+"일"+'</span>'}
				}else{
					return'';
				}
		}
			...
			for(var i=0;i<toDayElement.length;i++){
				...
				toDayElement[i].onclick=function(e){
					...
					if($(this).parents('.t-check-in').hasClass('t-check-in')===true){
						...
						data_utc_in=get_utc;data_utc_out=dataUTC[1];
						if(get_utc===settings.startDate&&$(this).hasClass('checkin')===false&&dataUTC[0]>=settings.startDate){
					...
					if($(this).parents('.t-check-out').hasClass('t-check-out')===true){
						if($(this).hasClass('checkin')===true){
							return;
						}
						if($(this).parents('.t-datepicker').find('.checkin').length===0&&$(this).hasClass('checkout')===true){
							return;
						}
						...
						function checkNumNight(pr_el,pr_date_utc){
							...
							if($(pr_el).parents('.t-check-in').hasClass('t-check-in')===true){
								...
								if(el_hover===settings.startDate){
									if($(pr_el).parents('.t-check-in').find('.checkout').length===0&&$(pr_el).parents('.t-check-in').find('.checkin').length===0){
		...
		function callEventClick(pr_callback,pr_date_utc){
			this_el.find('.t-dates').on('click',function(e){
				...
				if($(this).parents('.t-datepicker').find('.t-input-check-in').val()==='null'&&$(this).parents('.t-check-out').find('.t-input-check-out').val()==='null'){
					click=$(e.target).parents('.t-datepicker').find('.t-check-in');$(e.target).trigger('clickDateCI',[pr_date_utc]);
				}
	...
	$.fn.tDatePicker=function(pr_el,options){
		...
		function setThemeCheckDate(pr_title,pr_class,pr_data_utc,pr_input,pr_fm_input,pr_requied){
			return'<div class="t-dates t-date-'+pr_class+'">'+settings.iconDate+'<label class="t-date-info-title">'+pr_title+'</label>'+
...
if(settings.showDateTheme==='dd'){return'<span class="t-day-'+pr_class+'"> '+check_num_10(d.getDate())+'</span>'
	}else if(settings.showDateTheme==='dd/mm'||settings.showDateTheme==='dd-mm'){
		return'<span class="t-day-'+pr_class+'"> '+check_num_10(d.getDate())+'</span>'+'<span class="t-month-'+pr_class+'"> '+showMonths+' </span>'
	}else{
		return'<span class="t-year-'+pr_class+'"> '+check_num_10(d.getFullYear())+"년"+'</span>'+'<span class="t-month-'+pr_class+'"> '+showMonths+' </span>'+'<span class="t-day-'+pr_class+'"> '+check_num_10(d.getDate())+"일"+'</span>'}
...
return pr_date;}
function getDateUTC(pr_in,pr_out){var Array_In_Out=['check-in','check-out'];Array_In_Out.forEach(function(e){var label_title=settings.titleCheckIn;var getDay=pr_in;var Input='checkin'
...
if(this_picker.find('.t-input-check-in').val()!=='null'||settings.toDayHighlighted!==false){toDayElement[i].className='checkin';}}
if(Number(dayselect)===dataUTC[1]){var this_picker=$(toDayElement[i]).parents('.t-datepicker')
if(this_picker.find('.t-input-check-out').val()!=='null'||settings.nextDayHighlighted!==false){toDayElement[i].className='checkout';}}
...
var d=new Date(dataUTC[1]);var limitdate=Date.UTC(d.getFullYear(),d.getMonth(),d.getDate()-settings.limitDateRanges);data_utc_in=get_utc;data_utc_out=dataUTC[1];if(get_utc===settings.startDate&&$(this).hasClass('checkin')===false&&dataUTC[0]>=settings.startDate){data_utc_out=dataUTC[0];}
...
if($(this).parents('.t-check-out').hasClass('t-check-out')===true){if($(this).hasClass('checkin')===true){return;}
if($(this).parents('.t-datepicker').find('.checkin').length===0&&$(this).hasClass('checkout')===true){return;}
$(pr_el).trigger('selectedCO',Number(get_utc))
...
if($(pr_el).parents('.t-check-in').hasClass('t-check-in')===true){var nd=new Date(pr_date_utc[1]);if(el_hover===settings.startDate){if($(pr_el).parents('.t-check-in').find('.checkout').length===0&&$(pr_el).parents('.t-check-in').find('.checkin').length===0){nd=new Date(pr_date_utc[0]);}}
...
if(settings.showDateTheme==='dd'){return'<span class="t-day-'+pr_class+'"> '+check_num_10(d.getDate())+'</span>'
		...
		return'<span class="t-day-'+pr_class+'"> '+check_num_10(d.getDate())+'</span>'+'<span class="t-month-'+pr_class+'"> '+showMonths+' </span>'}else{
			return'<span class="t-year-'+pr_class+'"> '+check_num_10(d.getFullYear())+"년"+'</span>'+'<span class="t-month-'+pr_class+'"> '+showMonths+' </span>'+'<span class="t-day-'+pr_class+'"> '+check_num_10(d.getDate())+"일"+'</span>'}
}else{return'';}}
function convertDateUTC(pr_date_utc){var date=new Date(pr_date_utc);var date_utc=Date.UTC(date.getFullYear(),date.getMonth(),date.getDate());if(date_utc===0){date_utc=null};return date_utc;}
function convertFormatDf(pr_date){if(typeof(pr_date)==='object'&&pr_date!==null){return pr_date=convertDateUTC(pr_date)}
